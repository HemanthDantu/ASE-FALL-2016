...
var app = express();
var result={'body': []};
var url = 'mongodb://user:user123@ds051838.mlab.com:51838/hemanthdantu';
app.use(cors());
...
app.post('/register', function (req, res) {
    ...
    var insertDocument = function(db, data, callback) {
        db.collection('ron').insertOne( data, function(err, result) {
            if(err)           {
...
app.post('/get-data',function (req,res) {
    ...
    var findUser = function(db, callback) {
        var cursor =db.collection('ron').find();
...
app.post('/update',function (req,res) {
    ...
    var updateDocument = function(db, data, callback) {
        db.collection('ron').updateOne({"fname":id},{$set:item}, function(err, result) {
            if(err)            {
                res.write("Registration Failed");
                res.end();            }
...
app.post('/delete', function(req, res) {
    ...
    MongoClient.connect(url, function(err, db) {
        if(err)        {
            res.write("Registration Failed");
            res.end();        }
        db.collection('ron').deleteOne({"fname": id}, function(err, result) {
